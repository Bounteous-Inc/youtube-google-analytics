...
// Track with Google Classic Analytics (Deprecated)
var enableGoogleClassicAnalytics = false;

// Track with classic Google Universal Analytics
var enableGoogleAnalytics = true;

// Track with Mixpanel (https://mixpanel.com)
    ...
    for (var j = 0; j < videoArray.length; (j++)) {
        ...
        if (videoArray[j] === videoID) {
            ...
            if (event.data === YT.PlayerState.PLAYING) {
                if(enableGoogleClassicAnalytics) {
                ...
                if (enableMixpanel) {
                    mixpanel.track("Video Play", {"videoID":videoID, "videoTitle":videoTitle[j]});
                }
                // thy video plays
            ...
            if (event.data === YT.PlayerState.ENDED) {
                ...
                if (enableMixpanel) {
                    mixpanel.track("Video End", {"videoID":videoID, "videoTitle":videoTitle[j]});
                }
            }
            ...
            if (event.data === YT.PlayerState.PAUSED && pauseFlagArray[j] !== true) {
                ...
                if (enableMixpanel) {
                    mixpanel.track("Video Pause", {"videoID":videoID, "videoTitle":videoTitle[j]});
                }
            ...
            if (event.data === YT.PlayerState.BUFFERING) {
                ...
                if (enableMixpanel) {
                    mixpanel.track("Video Buffering", {"videoID":videoID,"videoTitle":videoTitle[j]}); 
                }
            }
            // and should it cue
            ...
            if (event.data === YT.PlayerState.CUED) {
                ...
                if (enableMixpanel) {
                    mixpanel.track("Video Cueing", {"videoID":videoID,"videoTitle":videoTitle[j]}); 
                }
            }
    ...
    jQuery('iframe').each(function() {
        ...
        if ($(this).attr('src')) {
            ...
            if (reloadFrames) {
                ...
                var SourceCheckA = vidSrc.match(/(?:https?:)?\/\/www\.youtube\.com\/embed\/([\w-]{11})(\?)?/);
                if (SourceCheckA[2] === "?") {
                    // it has the beast
                    // we must be cautious
                    // has it been thus gifted for jsapi magic?
                    var SourceCheckB = vidSrc.match(/enablejsapi=1/);
                    if (SourceCheckB) {
                        // it has the gift
                        // accept it and move on
                        console.log("SourceCheckB");
                    } else {
                    ...
                    var regex3 = /origin=.*/;
                    var SourceCheckC = vidSrc.match(regex3);
                    if (SourceCheckC) {
                        for (var j = 0; j < SourceCheckC.length; (j++)) {
                            // Ah but it has an origin and we
                            // shall change it
                            // waving our hands we create a new
                            // origin
                            // as there is no place like
                            // window.location.hostname
                            vidSrc = vidSrc.replace(regex3, "origin=" + window.location.hostname);
                        }
                    } else {
                    ...
                    // It is missing the mark of the parameter
                    // entirely
                    // this is not unexpected
                    // we shall garb it in the clothing of our
                    // homeland
            ...
            var matches = vidSrc.match(/(?:https?:)?\/\/www\.youtube\.com\/embed\/([\w-]{11})(?:\?.*)?/);
            // Should the former reg provide a match
            // it shall appear in an array of matches
            if (matches && matches.length > 1) {
                // we now place the beating heart of the youtube
                // id
                ...
                getRealTitles(i);
                // And for this, I am no longer nothing, I am
                // more
                i++;
            }
        }
